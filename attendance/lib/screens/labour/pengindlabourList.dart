...
class PendingAtHRLabour extends StatefulWidget {
  const PendingAtHRLabour({super.key});

  @override
  State<PendingAtHRLabour> createState() => _PendingAtHRState();
}

class _PendingAtHRState extends State<PendingAtHRLabour> {
...
Future<void> gatTeamMemberList() async {
  ...
  try {
    ...
    return member.status == "Pending" && // Assuming 'status' is the field for status
  ...
  if (name.isEmpty) {
    ...
    filteredTeamMembers.value = teamAttendanceModel.value.tDS?.where((member) {
      ...
      bool matchesOtherConditions = member.status == "Pending" && // Assuming 'status' is the field for status
           member.faceImage != null &&
           member.faceImage!.isNotEmpty &&
           member.faceImage != "https://rapi.railtech.co.in/";
                                            ...
                                            child: Padding(padding:const EdgeInsets.symmetric(horizontal: 5,vertical: 5),
                                              child: Column(
                                                children: [
                                                  ...
                                                  //               filteredTeamMembers![index].faceImage !=""?"": teamAttedanceModel!.tDS![index].faceImage!
                                                  //               );
                                                  //         },
                                                  //         child: Container(
                                                  //           decoration:BoxDecoration(
